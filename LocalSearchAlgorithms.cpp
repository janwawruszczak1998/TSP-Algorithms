            ...
            while(rep-->0){
                ...
                if(step_val < result){  //jesli poprawilismy, wyzeruj wskaznik stagnacji i aktualizuj rezultat
                    ...
                    if(stagnation < g->get_rank()) {
                        ...
                        do{
                            f_inv = rand() % g->get_rank();
                            s_inv = rand() % g->get_rank();
                        }while(f_inv == s_inv);
                ...
                double prob =   double( (double)rand() / (double)(RAND_MAX));
            ...
            for (int f_pos = 0; f_pos < g->get_rank(); ++f_pos) {
                ...
                for (int s_pos = f_pos + 1; s_pos < g->get_rank(); ++s_pos) {
                    ...
                    if (curr_val < next_step_val) {              //jesli rozwiazanie lepsze niz lokalne (w ramach ruchu)
                        ...
                        stagnation++;
                        if(stagnation < g->get_rank() * sqrt(g->get_rank() )) {
                            int f_inv = 0;  //wyznaczenie pierwszego miejsca inverta
                            int s_inv = 0;  //wyznaczenie drugiego miejsca inverta
                            do{
                                f_inv = rand() % g->get_rank();
